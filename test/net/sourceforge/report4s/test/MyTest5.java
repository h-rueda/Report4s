...
public class MyTest5 {
	...
	public void search() {
		...
		Report4s.logEvent("click on select colombia", true);
		sel.selectByValue("colombia");
		
		Report4s.logEvent("click on select france", true);
		sel.selectByValue("france");

		Report4s.logEvent("deselect poland", true);
		sel.deselectByValue("poland");

		Report4s.logEvent("deselect all", false);
		sel.deselectAll();

		elem = eventDriver.findElement(By.linkText("Google"));
		Report4s.logEvent("click link", true);
		elem.click();
		
		Report4s.logEvent("Navigate back", true);
		eventDriver.navigate().back();

		elem = eventDriver.findElement(By.name("submit"));
		Report4s.logEvent("click button", true);
		elem.click();
		
		eventDriver.navigate().back();
		
		elem = eventDriver.findElement(By.name("submit"));
		Report4s.logMessage(Level.INFO, "I can log an info message");
		Report4s.logMessage(Level.WARNING, "I can log a warning message", driver);
		Report4s.logMessage(Level.DEBUG, "I can log a debug message", driver, elem);
		Report4s.logMessage(Level.PASSED, "message in <b>bold</b>", driver, elem, 20);
	}
